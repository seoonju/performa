...
module.exports = Class.create({
	...
	api_upload_file: function(args, callback) {
		...
		var sanitizedPath = Path.normalize(args.query.path).replace(/^(\.\.[\/\\])+/, '');
		if (sanitizedPath.includes('..')) {
			return self.doError('file', "Invalid path found in request.", callback);
		}
		...
		this.loadSession(args, function(err, session, user) {
			...
			var storage_key = 'files/' + sanitizedPath;
